...
var gameEvents = function(socket, gameId) {
  ...
  return {	
    ...
    playerKicked: function(result) {
	  var packet = result;
      emitGameEvent('kick', packet, 'player');
    },
...
function broadcastOnKick(socket, gameId, userId) {
  socket.on('kick', function (packet, callback) {
	console.log("broadcastOnKick "+JSON.stringify(packet))
	gt.Main.kickPlayer(userId, gameId, packet.userId, callback);
  });	
}

function listenOnGameCommand(socket, gameId, userId) {
  socket.on('game', function (packet, callback) {
	gt.Main.dispatchCommand(packet, gameId, userId, callback);
  });
}
...
function emitOnGameEvents(socket, gameId) {
  ...
  gt.Main.on('playerKicked', ge.playerKicked);
...
function removeGameEvents(gameEvents) {
  ...
  var pk = gameEvents.playerKicked;
  ...
  gt.Main.removeListener('playerKicked', pk);
}
...
function initLobbySocket(socket, userId, nick, gameId) {
  ...
  broadcastOnKick(socket, gameId, userId);
