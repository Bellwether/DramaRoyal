...
function broadcastOnChat(socket, userId, nick) {
  var broadcastChatMessage = function (msg, callback) {
  ...
  socket.on('chat', broadcastChatMessage);
}
...
var connectToLobby = function (socket) {
  socket.once('authorizeUser', function (sessionId) {
	authenticateUserSession(sessionId, function(err, doc) {
	  if (err) console.log("ERROR IN authenticateUserSession: "+err)
	
      if (doc) {
	    console.log("COMET authenticated user "+doc.avatar.nick+" ("+doc._id +")");
	    joinLobby(socket, doc._id, doc.avatar.nick);
      } else {	
		console.log("ERROR COMET failed to authenticate user: "+err);
