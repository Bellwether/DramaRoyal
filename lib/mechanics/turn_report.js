...
var describer = {
  ...
  describeActives: function(outcome) {
    for (var idx = 0; idx < describer.actives.length; idx++) {
      ...
      var result = new tr.TurnResult();	
      result.addTargets(uid, nick);
      result.esteem = active.esteem;	
	
	  if (result.action.cmd === 'med') result.trackPlayerMedUse(uid);
	  ...
	  function describeGrabs() {
        ...
        var isMedicating = active.action.cmd === 'med';
        var isScratching = active.action.cmd === 'scratch';
	  ...
	  function describeScratches() {
		...
		if (isAlsoGrabbed) description = description + 'while grabbed ';
	  ...
	  function describeCowardice() {
	    if (active.isCoward()) {
		  description = description + ',  at last shamed further for cowering again and again.';	
		}
	  };
