  ...
  describeTattles: function(outcome) {
	...
	function describeManyTattles() {
      ...
      if (result.humiliations) {
	    ...
	    for(var idx = 0; idx < result.humiliations.length; idx++) {
		  var nick = result.humiliations[idx];
		  description = (idx === 0 ? nick+' ' : description+' and '+nick );
	    }
  ...
  describePassives: function(outcome) {
    for (var idx = 0; idx < describer.passives.length; idx++) {	
	  ...
	  if (action === 'nothing') {
        ...
        if (passive.isCoward()) description = description + ' and was shamed for her groveling.';
	  }
	  else if (action === 'med') {
  ...
  describeActives: function(outcome) {
    for (var idx = 0; idx < describer.actives.length; idx++) {
	  ...
	  function describeScratches() {
		...
		var scratchers = [];
		var didAttackersHitTarget = active.isEffective();
		
		scratches.forEach(function(scratch){
		  scratchers.push(scratch.nick);
		  result.addAttacker(scratch.userId, scratch.nick, 'scratch', didAttackersHitTarget);
		});
	  ...
	  function describeCowardice() {
	    if (active.isCoward()) {
		  description = description + ' and was shamed for her groveling.';
		}
	  };
