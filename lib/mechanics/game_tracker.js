...
GameManager.prototype.joinGame = function(gameId, userId, callback){
  var self = this;

  var playerJoined = function(err, doc){
    if (err) {
	  callback(err);
    } else {
	  self.emit('playerJoined', doc, game._id);
	  callback(err, doc);
    }
  }		
  var playerCanJoin = function() {
    ...
    gm.findOne(params, function(err, doc){
	  if (doc) {
		doc.createPlayer(userId, playerJoined);
	  } else {
	    callback(err);
	  }
    });    	
