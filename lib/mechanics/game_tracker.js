	...
	if (doc) {
	  doc.quitPlayer(userId, function(err, doc) {
		if (doc.hasActivePlayers()) {	
		  console.log("quitGame doc.hasActivePlayers()")
	      self.emit('playerQuit', {'_id': userId}, gameId );
		  console.log("quitPlayer: "+JSON.stringify(doc));
	    } else if (doc.hasPlayers()) {
		  console.log("quitGame doc.hasPlayers()")
		  doc.quit( function(err){
		    console.log("doc.quit!!!!!!")
		    self.emit('gameEnded', doc);
			console.log("quitPlayer: "+JSON.stringify(doc));
		  });
	    } else {
		  console.log("quitGame gameEnded")
		  self.emit('gameEnded', doc);
		  doc.remove();
		};
