  ...
  this.processTattleResults = function() {
    ...
    if (playerTattled) {
	  ...
	  for(var key in results) {
	    ...
	    console.log("playerTattled result: "+JSON.stringify(result))
      ...
      if (!tattleWasEffective) {
	    console.log("!tattleWasEffective ("+tattlerId+": "+results[tattlerId]+")")
  ...
  this.processScratchResults = function() {
    for(var key in results) {
      ...
      if (scratches.length > 0) {
	    console.log("-- processScratchResults: check for cower: "+JSON.stringify(result.action))
