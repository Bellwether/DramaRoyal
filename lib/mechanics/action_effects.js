...
var EffectProcessor = function(turn, results, lastTurn) {
  ...
  this.processTattleResults = function() {
    ...
    if (playerTattled) {
	  ...
	  for(var key in results) {
	    ...
	    var isOwnPlayer = result.userId+'' === tattles[0].userId+'';
  ...
  this.processTeaseResults = function() {
    for(var key in results) {
      ...
      if (teases.length === 1) { // requires two or more teases to have effect
	    results[teases[0].playerId].defective();
      ...
      if (scratches.length > 0) {
	    if (result.action.cmd === 'cower') { 
	      ...
	      for(var idx = 0; idx < scratches.length; idx++) {
		    results[scratches[idx].playerId].defective();
 	      }
	    } else {
          ...
          // if player is licking, scratches do extra damage
	      if (result.action.cmd === 'lick') {
		    result.defective();
  ...
  this.processLickResults = function() {
    var licks = getActionsByType(turn.actions, 'lick');

    for(var idx = 0; idx < licks.length; idx++) {
      var key = licks[idx].userId;
      var result = results[key];
