...
function beginNewUserSession(req, callback){
  var token = req.facebookSession.token();
  var fbId = req.facebookSession.userId();
  var fbGraphFunction = req.fbGraph.user(token);

  usr.User.findOrCreateFromFacebook(fbId, token, fbGraphFunction, function(err, doc){
...
function getUserByOauthToken(req, callback) {
  if (req.facebookSession.token === undefined) {
	callback(false);
  } else {
  ...
  if (isWebPageRequest(req)) {
	getUserBySession(req, function(hasUser){ 
	  if (hasUser) {
		...
		getUserByOauthToken(req, function(hasUser){
		  if (hasUser) {
		    callback(true);
		  } else {
		    res.redirect('/auths');
