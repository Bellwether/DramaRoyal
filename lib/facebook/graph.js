...
module.exports.GraphAPI = function (){
  ...
  var api = {
    ...
    achieve: function(userId, achievement, removeAchievement) {
	  var token = cnfg.AppAccessToken;
	  var path = '/'+userId+'/achievements';
	  var params = {access_token: token, achievement: achievement.url};
	
	  var method = removeAchievement === true ? 'DELETE' : 'POST';
	  return api.graphCall(path, params, method);
    },
    clearRequest: function(userId, requestId) {
	  var token = cnfg.AppAccessToken;
	  var path = '/'+userId+'/apprequests/'+requestId;
	  var params = {access_token: token};
	
	  return api.graphCall(path, params, 'DELETE');
    }
  }	
